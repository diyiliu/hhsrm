...
$.formValidator = 
{
	...
	getInitConfig : function( validatorGroup )
	{
		if(jQuery_formValidator_initConfig!=null)
		{
		    ...
		    return jQuery_formValidator_initConfig;
		}
		return null;
	},
	...
	SetTipState : function(tipid,showclass,showmsg)
	{
		...
		{
			if(showclass == "onFocus")  //add by zhangzl 焦点移入时淡入提示信息
			{
				tip.removeClass();
	...
	HiddenTip : function(tipid,showclass)
	{
		var tip = $("#"+tipid);
		tip.fadeOut("fast");
	},
	...
	ShowMessage : function(returnObj)
	{
		...
		{		
			...
			{
				var elemValue = $.trim($("#"+returnObj.id).val());
				if(elemValue != ""){
					/*if(elemValue.length < setting.min){
						showmsg = (returnObj.minmsg==""? setting.onmin : returnObj.minmsg);
						setting.onerror = showmsg;
					}else if(elemValue.length > setting.max)
					{
						showmsg = (returnObj.maxmsg==""? setting.onmax : returnObj.maxmsg);
						setting.onerror = showmsg;
					}else{
					...
					showmsg = " ";
					if(settings[0].onshow.length > 0 && settings[1].min > 0){
						showclass = "onShow";
					}
				}
			}
		...
		{		
			...
			if(!intiConfig.alertMessage)
			{
				...
				*/
				...
				$.formValidator.HiddenTip(setting.tipid,"onSuccess");  //modiby by zhangzl 验证成功后隐藏提示框
			}
		}
	},
    ...
    GetLength : function(id)
    {
        ...
        switch(sType)
		{
			...
			case "radio": 
				...
				break;
		    case "select-one":
		        len = srcjo.get(0).options ? srcjo.get(0).value.length: -1;
				break;
			case "select-more":
				break;
	    }
	...
	PageIsValid : function (validatorGroup)
	{
		...
		{
			if(elem.settings.length > 0 && elem.settings[0].bind){
					...
					if (!returnObj.isValid) {
						...
						if (thefirstid == ""){
							...
							if($.trim(elem.value).length > 0){
								...
								thefirsterrmsg = (returnObj.emptymsg==""?returnObj.setting.onempty:returnObj.emptymsg)
							}
						}
					}
		...
		else
		{
			...
			//if(thefirstid!=""){$("#"+thefirstid).focus();}  //去除提醒前自动对焦点，统一用回调方式。主要防止没有确认就滚动上去
		}
...
$.fn.formValidator = function( msgOptions) 
{
	...
	return this.each(function()
	{
		...
		//初始化显示信息
		if(!initConfig.alertMessage && this.value ==""){
		...
		if (srcTag == "INPUT" || srcTag=="TEXTAREA")
		{
			...
			{   
				...
				$('#'+returnObj.id).attr("value",$.trim($('#'+returnObj.id).val()));
			});
		} 
		else if (srcTag == "SELECT")
		{
			...
			{	
				var returnObj = $.formValidator.OneIsValid(this.id,1);
				if(!initConfig.alertMessage){
					if($("#"+returnObj.id).get(0).options.selectedIndex < 0  || $("#"+returnObj.id).get(0).options[$("#"+returnObj.id).get(0).options.selectedIndex].value.length == 0){
						$.formValidator.SetTipState(tip,"onShow",setting.onshow);
					}
				}
			});
