...
$(document).ajaxError(function(event,xmlRequest,ajaxOptions,throwError){
	...
	//fh.alert('当前账户超时，请重新登录！',false,function(){
		if(xmlRequest.status == 302 || xmlRequest.status == 0){
			if(xmlRequest.responseText.lastIndexOf('/login.action')!=-1){
...
function clearForm(id) {
	...
	for (var i = 0; i < elements.length; i++) {
		...
		if (element.type == 'checkbox') {
		    $(element).parents("li:first").find("input[type='checkbox']:first").attr("checked",true);
		} 
		//console.log(element.type)
		if (element.type == 'radio') {
		   $(element).parents("li:first").find("input[type='radio']:first").attr("checked",true);
		} 
	}
...
function createLightbox(obj,id,isParent){
	...
	var url = getBasePath() + "/application/jsp/openImg/imgbox.jsp";
	commonOpenDialog("showimg","图片预览",width,height, url,null,id,null,parentId,null,false);
}

function downloadUrl(url)
{
	var frameId = "__download_frame__23u4asdf89023kjs__";
	var f = $("#" + frameId);
	if (f.length == 0)
	{
		f = $("<iframe style='display:none;'>").attr("id", frameId).appendTo($(document.body));
	}
	f.attr("src", url);
}