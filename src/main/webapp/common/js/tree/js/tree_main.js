...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        function buildtree(data, ht) {
            ...
            ht.push("<div class='bbit-tree-body_main'>"); // body ;
        ...
        function nodeclick(e) {
            ...
            // if (et.tagName == "IMG") {
            //hasClass("bbit-tree-elbow")
            var parent = $(et).parent();
            if($(et)[0].tagName == "SPAN"){
                parent = parent.parent();
            }
            et = parent.find("img")[0];
            // +号需要展开
            if ($(et).hasClass("bbit-tree-elbow-plus") || $(et).hasClass("bbit-tree-elbow-end-plus")) {
                    ...
                    if(ul.parent().parent().find("img.bbit-tree-elbow-minus").length>0){
                        closeAll(ul.parent().parent().find("img.bbit-tree-elbow-minus")[0]);
                    }
                    if(ul.parent().parent().find("img.bbit-tree-elbow-end-minus").length){
                        closeAll(ul.parent().parent().find("img.bbit-tree-elbow-end-minus")[0]);
                    }
                ...
                if (dfop.citem) {
                    ...
                    if($("#" + id + "_" + nid).hasClass("bbit-tree-node-leaf2")){
                        $(this).removeClass("bbit-tree-selected-2");
                    }else{
                        $(this).removeClass("bbit-tree-selected");
                    }
                }
                $("div .bbit-tree-selected").removeClass("bbit-tree-selected");
                $("div .bbit-tree-selected-2").removeClass("bbit-tree-selected-2");
                dfop.citem = item;
                $(this).addClass("bbit-tree-selected");
                $(this).addClass("bbit-tree-selected-2");
                if (dfop.onnodeclick) {
                    if (!item.expand) {
        ...
        function bindevent() {
            $(this).hover(function() {
                if($(this).hasClass("bbit-tree-node-leaf2")){
                    $(this).addClass("bbit-tree-node-over-2");
                }else{
                    $(this).addClass("bbit-tree-node-over");
                }
            }, function() {
                if($(this).hasClass("bbit-tree-node-leaf2")){
                    $(this).removeClass("bbit-tree-node-over-2");
                }else{
...
function getParentCount(node,i){
    if(node.parent){
        return getParentCount(node.parent,++i);
    }else{
        return i;
    }
}
...
function closeAll(et){
            if ($(et).hasClass("bbit-tree-elbow-minus") || $(et).hasClass("bbit-tree-elbow-end-minus")) {  // -  // 号需要收缩
                $(et).parent().next().hide();
                if ($(et).hasClass("bbit-tree-elbow-minus")) {
                    $(et).swapClass("bbit-tree-elbow-minus", "bbit-tree-elbow-plus");
                }
                ...
                $(et).parent().swapClass("bbit-tree-node-expanded", "bbit-tree-node-collapsed");
            }
        }