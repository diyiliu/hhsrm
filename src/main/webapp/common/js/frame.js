...
*Date:2012-06-26-
*Descript:页面框架公用js
*Tips:有问题请RTX或是mail 李宁（21楼 UED）
*Update:2012-07-18  10:17 
*Update:LN（Allison Lee）
*Descript:修改数据没有加载完成iframe高度的计算的问题
*Update:2012-07-31  09:54
*Update-Descript:添加页面页签个数控制，超过一定的页签个数，给出自定义需要删除的选项
*/
...
$(document).ready(function(){
	$("#Js_uiMenu > li > a.ui-menu-title").each(function(){
		...
		_this.live("click",function(){
            if(_this.parent().find("img.bbit-tree-elbow-minus").length>0){
                closeAll(_this.parent().find("img.bbit-tree-elbow-minus")[0]);
            }
            if(_this.parent().find("img.bbit-tree-elbow-end-minus").length>0){
                closeAll(_this.parent().find("img.bbit-tree-elbow-end-minus")[0]);
            }
...
$(document).ready(function(){
        ...
        {
        //<span class="lntab-icon"><img src="'+lnTabs[tt].tabImg+'" /></span>
        //<span class="close '+(lnTabs[tt].tabClose=="true"? " ":"hide")+'"></span><span class="tab_r"></span>
            ulModTag+='<li id="'+lnTabs[tt].tabID+'-head" class="ln_tabitem"><a href="javascript:void(0)" hidefocus="true" id="'+lnTabs[tt].tabID+'" class="'+(lnTabs[tt].tabClose=="true"? " ":"staylink")+'" >'+lnTabs[tt].tabh+'</a></li>'
        }
        ulModTag+='</ul>'   
    ...
    function addTabCon(attributes){
    	...
    	for(var t=0;t<tabL;t++)
    	{
    		...
    		{
    			isIn=true
    			if("整车下单审核"==attributes.title){isIn=false}
				if("配件下单审核"==attributes.title){isIn=false}
    			isInTab=tab.eq(t)
    		}
    ...
    function testTabLen(attributes){
		...
		if(maxSize<=hasSize)
		{
			...
			var choiceMainBox='<div class="choice-main-box" style="position:absolute; top:50%; left:50%; background:#fff; z-index:1000;margin-top:'+mainMarginTop+'px;"><div class="choice-main-hd">请选择需要关闭的项&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 全选/全不选 <input class="checkbox" id="checkall" type="checkbox"/></div><ul class="choice-list">'
			for(var i=0;i<hasSize;i++){
				var disabled=(i==0 ? "disabled" :"");
				var liItem= '' ;
				if(i==0){
					liItem='<li class="choice-item" index="'+i+'"><input class="checkbox" name="myCheck1" type="checkbox" '+disabled+'  /><span>'+$("#Js_LnTabCon").find(".ln_tabitem").eq(i).text()+'</span></li>'
				}else{
					liItem='<li class="choice-item" index="'+i+'"><input class="checkbox" name="myCheck" type="checkbox" checked '+disabled+'  /><span>'+$("#Js_LnTabCon").find(".ln_tabitem").eq(i).text()+'</span></li>'
				}
		...
		//	choiceMainBox+='</ul><div class="choice-main-fd"><a href="javascript:void(0)" id="Js_confrmCancel">取消</a><a id="Js_confrmYes" href="javascript:void(0)">确定</a></div></div>'
			...
			choiceMainBox+='<script>$("#checkall").click( function(){if(this.checked){$("input[name=\'myCheck\']").attr(\'checked\', true)}else{$("input[name=\'myCheck\']").attr(\'checked\', false)} } ); </script>'
			...
			$("#Js_confrmYes").live("click",function(e){
				...
				for(var i=0; i<selectedInput.length;i++){
                    ...
                    if($(".Js_lnTab > li").eq(tabIndex).hasClass("current"))
                    {
                        $(".Js_lnTab > li").eq(tabIndex-1).click();
                    }  
                    $(".Js_lnTab > li").eq(tabIndex).remove();
                    $("#Js_LnTabCon > div").eq(tabIndex).remove();
				}
                }catch(e){
                    alert(e.message);
                }
    ...
    function addTabNow(attributes){
            ...
            //<span class="lntab-icon"><img src="'+attributes.tabImg+'" /></span>
            //<span class="close '+(attributes.tabClose=="true"? " ":"hide")+'"></span><span class="tab_r"></span>
	    	var tabHd='<li id="'+attributes.id+'-head" class="ln_tabitem"><a href="javascript:void(0)" hidefocus="true" id="'+attributes.id+'">'+attributes.title+'</a></li>'
...
//    			   _this.parents("body").find(".ui-layout-toggler-west").click();
	...
	function bindCloseClick(){
      ...
      //注意 关闭后要重新计算当前li的索引
        ...
        //2013-05-13 chenyong 在手工关闭时，清空帧内保存的数据	  
        var id  = _this.parent().attr("id");
       	
       	fhdebug("frame id is ", id);
        var mch = /^(.*)\-head$/.exec(id);
        if(mch){
      	  var frameId = ["iframe", mch[1]].join("-");
      	  if(typeof FhFormSerializer != 'undefined' && FhFormSerializer.clearFrameValue){
      		  fhdebug("clearFrameValue ", frameId);
      		  FhFormSerializer.clearFrameValue(frameId);
      	  }
        }
   		//聚焦上一个    	
