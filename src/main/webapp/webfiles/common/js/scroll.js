...
Scroller.prototype = {
	initialize : function(idScroller, idScrollMid, options) {
		...
		this.pause = 0; // 定高(宽)
		this.side = 0; // 参数
		// alert(oScroller.style.width+"----------"+oScroller.style.height)
		// 用于上下滚动
		this.heightScroller = parseInt(oScroller.style.height)
				|| oScroller.offsetHeight;
		this.heightList = oScrollMid.offsetHeight;
		//alert(this.heightScroller+"----------"+this.heightList)

		// 用于左右滚动
		this.widthScroller = parseInt(oScroller.style.width)
				|| oScroller.offsetWidth;
		this.widthList = oScrollMid.offsetWidth;
		// alert(this.widthScroller+"----------"+this.widthList+"========="+oScroller.clientWidth+"----------"+oScroller.scrollWidth)
		// js取不到css设置的height和width

		oScroller.style.overflow = "hidden";
		oScrollMid.appendChild(oScrollMid.cloneNode(true));
		oScrollMid.appendChild(oScrollMid.cloneNode(true));
		// alert(oScrollMid.innerHTML)
	...
	ScrollUpDown : function() {
		...
		//alert(this.scroller.scrollTop)
		this.scroller.scrollTop = this.GetScroll(this.scroller.scrollTop,
				this.heightScroller, this.heightList, this.options.PauseHeight);
	...
	ScrollLeftRight : function() {
		...
		this.scroller.scrollLeft = this.GetScroll(this.scroller.scrollLeft,
				this.widthScroller, this.widthList, this.options.PauseWidth);
